       ...
       {cartItems.length === 0 && <div className = "empty-cart"> Your Cart is Empty</div>}
        {cartItems.map((item) => (
          <div key = {item.id} className = "item-list">
            <img className = "item-image list" src = {item.image} alt = {item.name}></img>
            <div className = "item-name list">{item.name}</div>
            <div className = "item-qty list">Qty: {item.qty}</div>
            <div className = "item-price list">Rs. {item.price.toFixed(2)} * {item.qty} </div>
            <button className ="add-btn" onClick  = {() => onAdd(item)}><FaPlus/></button>
            <button className ="remove-btn" onClick = {() => onDelete(item)}><AiTwotoneDelete /></button>
          </div>
